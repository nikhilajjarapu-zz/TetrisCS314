...
/**
 * JTetris presents a tetris game in a window. It handles the GUI and the
 * animation. The Piece and Board classes handle the lower-level computations.
 */
public class JTetris extends JComponent
{
	...
	/**
	 * Creates a Window, installs the JTetris or JBrainTetris, checks the testMode
	 * state, install the controls in the WEST.
	 */
	...
	{ new TetrisPiece(PieceType.STICK), new TetrisPiece(PieceType.SQUARE), new TetrisPiece(PieceType.T),
			new TetrisPiece(PieceType.LEFT_L), new TetrisPiece(PieceType.RIGHT_L), new TetrisPiece(PieceType.LEFT_DOG),
	 ...
	 * once with a Board.Action and the timer calls it with the verb DOWN to move
	 ...
	 * blocks in the board to (x,y) coords that count pixels.
	 *
	...
	public void paintComponent(Graphics g)
	{
		...
		for (int x = 0; x < bWidth; x++)
		{
			...
			for (int y = 0; y < bHeight; y++)
			{
				...
				if (currentPiecePositions.contains(new Point(x, y)))
